<x-layout>
        ...
        <div class="flex mt-16 justify-between">
            ...
            <div class="flex items-center justify-between relative flex-row-reverse animation-edit">
                ...
                <div class="h-full flex items-center">
                    <p class="text-neutral-400 text-sm cursor-pointer hover:text-white">EDITAR</p>
                </div>
            </div>
        </div>
            ...
            @if ($fourItems)
                ...
                @foreach ($fourItems as $notes)
                    ...
                    <x-cardplanejamento id_card='{{ $i }}' img_banner="{{ $notes->image }}"
                        nomeanotacao="{{ $notes->name }}" infos="{{ $notes->hour }}"></x-cardplanejamento>
                    @php
                        $i++;
                    @endphp
                @endforeach
    ...
    <x-list-item>
        <form action="{{ route('note.index') }}" method="GET" class="w-full">
            <input type="text" name="search" value="{{ $search }}" placeholder="Buscar notas" class="bg-transparent outline-none border-b text-white p-1">
            <button type="submit" class="ml-4"><?xml version="1.0" encoding="utf-8"?>
